	...
	for i, start in enumerate(start_list):
		...
		for j, kw in enumerate(kw_y_list):
			for k, area_item in enumerate(area_list):
				try:
					save_name_30_30 = "../result_h/scatter/sit_by_csv/" + csv_kind_list[0] + "/" + plot_y_list[j] + "/" + area_item + "/" + "lmplot_coastal_2_" + str(start)[:6] + ".png"
					lm = sns.lmplot(x="sit_30", y=kw[0], col="coastal_region_2", data=data_30[data_30["area_label"]==k], size=3, fit_reg=True)
					lm.set(ylim=y_lim_list[j])
					plt.savefig(save_name_30_30, dpi=500)
					plt.close()
				except:
					print("Skipped plot: {}".format(csv_kind_list[0] + "/" + plot_y_list[j] + "/" + area_item))
					plt.close()

				try:
					save_name_30_90 = "../result_h/scatter/sit_by_csv/" + csv_kind_list[1] + "/" + plot_y_list[j] + "/" + area_item + "/" + "lmplot_coastal_2_" + str(start)[:6] + ".png"
					lm = sns.lmplot(x="sit_30", y=kw[1], col="coastal_region_2", data=data_90[data_90.area_label==k], size=3, fit_reg=True)
					lm.set(ylim=y_lim_list[j])
					plt.savefig(save_name_30_90, dpi=500)
					plt.close()
				except:
					print("Skipped plot: {}".format(csv_kind_list[1] + "/" + plot_y_list[j] + "/" + area_item))
					plt.close()

				try:
					save_name_30_30_w_iw = "../result_h/scatter/sit_by_csv/" + csv_kind_list[2] + "/" + plot_y_list[j] + "/" + area_item + "/" + "lmplot_coastal_2_" + str(start)[:6] + ".png"
					lm = sns.lmplot(x="sit_30", y=kw[2], col="coastal_region_2", data=data_30_w_iw[data_30_w_iw.area_label==k], size=3, fit_reg=True)
					lm.set(ylim=y_lim_list[j])
					plt.savefig(save_name_30_30_w_iw, dpi=500)
					plt.close()
				except:
					print("Skipped plot: {}".format(csv_kind_list[2] + "/" + plot_y_list[j] + "/" + area_item))
					plt.close()
