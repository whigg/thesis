...
def get_1day_ice_data(ice_file_name):
	...
	iw_idx_t = idx_t
	return pd.DataFrame({"iw_u": iw_u, "iw_v": iw_v, "iw_speed": iw_speed, "iw_idx_t": iw_idx_t})
...
def get_1day_ic0_data(ic0_file_name, grid900to145_file_name):
	...
	return pd.DataFrame({"ic0_145": ic0_145, "ic0_idx_t": ic0_idx_t})
...
def get_w_regression_data(wind_file_name, ice_file_name, coeff_file_name):
	...
	"""
	w_data = np.array(get_1day_w_data(wind_file_name))
	w_u, w_v, w_speed = w_data[:,0], w_data[:,1], w_data[:,2]
	iw_data = np.array(get_1day_ice_data(ice_file_name))
	iw_u, iw_v, iw_speed, iw_idx_t = iw_data[:,0], iw_data[:,1], iw_data[:,2], iw_data[:,3]
	...
	data = np.c_[data_index_145, A_by_day, theta_by_day]
	data = pd.DataFrame(data)
	data.columns = ["data_idx", "A_by_day", "theta_by_day"]
