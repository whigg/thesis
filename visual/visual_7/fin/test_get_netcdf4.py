...
#地衡風データを重ねて可視化する．テスト用
df_w = np.array(pd.read_csv("../data/ecm030101.csv", header=None))
w_u = df_w[:,0]
w_v = df_w[:,1]
w_slp = df_w[:,2]
w_slp1 = np.reshape(w_slp, (145, 145), order="F")
...
#地衡風の可視化
m.contourf(x1, y1, w_slp1, cmap=plt.cm.jet)
m.colorbar(location="bottom")
#m.quiver(x, y, w_u, w_v)
