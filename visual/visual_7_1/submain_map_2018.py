...
def plot_anomaly_points():
	...
	for i, start in enumerate(start_list):
		...
		data_list = [np.array(data_30["anomaly"]), np.array(data_90["anomaly"])]
		for j in range(2):
			ax = axes[j]
			m = Basemap(lon_0=180, boundinglat=50, resolution='l', projection='npstere')
			m.ax = ax
			...
			data1 = np.reshape(data, (145,145), order='F')
			im = m.pcolormesh(xx_ex-dx1, yy_ex+dy1, data1, cmap=plt.cm.jet, vmax=1, vmin=0)
			fig.colorbar(im, ax=ax)
			#m.colorbar(location='bottom')
		save_name = dirs + str(start)[:6] + ".png"
		print(save_name)
