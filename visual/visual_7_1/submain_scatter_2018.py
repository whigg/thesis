		...
		for j, kw in enumerate(kw_y_list):
			for k, area_item in enumerate(area_list):
				try:
					...
					lm = sns.lmplot(x="ic0_30", y=kw[0], col="coastal_region_2", data=data_30[data_30["area_label"]==k], size=3, fit_reg=True)
				...
				try:
					...
					lm = sns.lmplot(x="ic0_30", y=kw[1], col="coastal_region_2", data=data_90[data_90.area_label==k], size=3, fit_reg=True)
					#lm.set(ylim=y_lim_list[j])
				...
				try:
					...
					lm = sns.lmplot(x="ic0_30", y=kw[2], col="coastal_region_2", data=data_30_w_iw[data_30_w_iw.area_label==k], size=3, fit_reg=True)
					#lm.set(ylim=y_lim_list[j])
