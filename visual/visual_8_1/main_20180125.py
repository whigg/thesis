...
def ts_all_with_trend(num):
	...
	#for area_index in [16]:
		...
		for item in date_7_12:
			ax.axvline(item, color='lime', linestyle='-', lw=0.75, alpha=0.75)
		ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
		plt.xticks(rotation=30)
		plt.tick_params(labelsize=14)
		plt.grid(True)
		plt.savefig(dirs + "A_no_std_area_" + str(area_index) + ".png", dpi=200)
		plt.close()
		...
		plt.tick_params(labelsize=14)
		plt.grid(True)
		plt.savefig(dirs + "theta_no_std_area_" + str(area_index) + ".png", dpi=200)
		plt.close()
		"""
		fig, ax = plt.subplots(1, 1)
		fig.figsize=(12, 9)
